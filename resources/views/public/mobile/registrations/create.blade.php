<x-mobile-layout :title="__('registrations.subtitle', ['tournament' => $tournament->name])">

@php($requiredFields = $tournament->requiredRegistrationFields())
@php($profile = auth()->user())

<h1 class="text-xl font-bold mb-1">{{ __('registrations.title') }}</h1>
<p class="text-gray-500 mb-4 text-sm">{{ __('registrations.subtitle', ['tournament' => $tournament->name]) }}</p>

<div class="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
    <form method="POST"
          action="{{ mroute('registrations.store', ['tournament' => $tournament->id]) }}"
          class="space-y-5">
        @csrf
