...
import { isFuturePetanqueAktuellTournament, mapPetanqueAktuellTournament, parsePetanqueAktuellCalendar, petanqueAktuellCalendarUrl, petanqueAktuellPageUrls } from './petanque-aktuell-core.js';
...
const SECURITY_HEADERS = {
  'Content-Security-Policy':
    "default-src 'self'; script-src 'self' https://challenges.cloudflare.com; style-src 'self'; img-src 'self' data: https://*.basemaps.cartocdn.com; connect-src 'self' https://challenges.cloudflare.com; font-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; frame-src https://challenges.cloudflare.com; form-action 'self'; upgrade-insecure-requests",
...
async function upsertPetanqueAktuellTournament(env, entry, ownerId, now, searches) {
  const db = env.DB;
  ...
  if (existing) {
    ...
    await db.prepare(`UPDATE tournaments SET name = ?, club = ?, date = ?, start_time = ?, location = ?, description = ?, formation = ?, formation_other = ?, website_url = ?, flyer_url = ?, status = 'registration', visibility = 'public', registration_enabled = 0, updated_at = ? WHERE id = ?`)
  ...
  const id = crypto.randomUUID();
  await db.batch([
    db.prepare(`INSERT INTO tournaments (id, owner_id, creator_id, name, date, start_time, location, description, type, formation, formation_other, registration_type, status, visibility, registration_enabled, club, website_url, flyer_url, timezone, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'formule_x', ?, ?, 'forme', 'registration', 'public', 0, ?, ?, ?, 'Europe/Berlin', ?, ?)`)
...
async function importPetanqueAktuellTournaments(request, env, user) {
  ...
  for (const key of externalKeys) {
    try { result[await upsertPetanqueAktuellTournament(env, selected.get(key), user.id, now, searches)] += 1; }
...
async function requestClubEditor(db, clubId, userId) {
  const club = await db.prepare('SELECT id FROM clubs WHERE id = ?').bind(clubId).first(); if (!club) throw new HttpError(404, 'Verein nicht gefunden');
  await db.prepare('INSERT OR IGNORE INTO club_editor_requests (club_id, user_id, created_at) VALUES (?, ?, ?)').bind(clubId, userId, new Date().toISOString()).run();
  return json({ ok: true });
}
...
async function createBoulePlace(request, db, clubId, user, countryCode) {
  await assertClubEditor(db, clubId, user); const input = await placeInput(await readJson(request), countryCode); const id = crypto.randomUUID(); const now = new Date().toISOString();
  await db.prepare("INSERT INTO boule_places (id, club_id, name, address, latitude, longitude, court_count, description, accessible, facilities, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)").bind(id, clubId, input.name, input.address, input.latitude, input.longitude, input.courtCount, input.description, input.accessible ? 1 : 0, input.facilities, now, now).run();
  return json({ id }, 201);
}
